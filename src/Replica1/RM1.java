...
import Replica1.Server.Server;
import Replica1.DMTBSapp.DMTBSHelper;
import Replica1.DMTBSapp.DMTBSOperations;
...
public class RM1 {
    ...
    private static void initial_send_list(Integer begin, Integer end, String RmNumber) {
        ...
        Message message = new Replica1.Message(0, list, "03", begin.toString(), end.toString(), RmNumber, "Null", "Null", 0, 0);
...
//                message_q.clear();
    ...
    private static String requestToServers(Message input,NamingContextExt ncRef) throws Exception {
        ...
        if (input.userID.substring(3, 4).equalsIgnoreCase("A")) {
            if (input.Function.equalsIgnoreCase("addMovieSlot")) {
                String response = obj.addMovieSlots(input.newMovieID, input.newMovieName, input.bookingCapacity);
    ...
    public static void reloadServers(NamingContextExt ncRef) throws Exception {
        for (ConcurrentHashMap.Entry<Integer, Replica1.Message> entry : message_list.entrySet()) {
