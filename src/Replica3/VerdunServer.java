...
class VERImpl extends DMTBSPOA {
    ...
    public void serverLog(String user, String requestType, String params, String status, String result)
        ...
        log_file = log_dir + "\\src\\Replica3\\log\\server\\verdun_logs.txt"; // log file path
    ...
    public String[] getBookingSchedule(String customerID) {
        ...
        for (Map.Entry mapElement : app_data.entrySet()) {
            ...
            for (Map.Entry mapSubElement : temp.entrySet()) {
                ...
                while (cap_data.contains(customerID)) {
                    if (!(result.contains("Verdun Movies : "))) {
                        result.add("Verdun Movies : ");
                    }
        ...
        if ((customerID.substring(0, 3)).equals("VER")) {
...
public class VerdunServer {
    ...
    public static void main(String[] args) {
        ...
        try {
            ...
            VERImpl verImpl = new VERImpl();
            verImpl.setORB(orb);

            // get object reference from the servant
            org.omg.CORBA.Object ref = rootpoa.servant_to_reference(verImpl);
            ...
            Runnable UDPServer = new Runnable() {
                ...
                public void run() {
                    // TODO Auto-generated method stub
                    receiveRequestFromAnotherServer(verImpl);
                }
            };
    ...
    public static void receiveRequestFromAnotherServer(VERImpl verobj) {
        String result = "";
        DatagramSocket ds = null;
        try {
            ds = new DatagramSocket(4651);
            byte[] receive = new byte[5000];
            System.out.println("UDP Server for Verdun is running on port : 4651");
            while (true) {
                ...
                if (func.equals("list_movieslots")) {
                    ...
                    ls = verobj.listMovieAvailabilitySupport(parameters.get(0));
                    result = String.join(",", ls).trim();
                } else if (func.equals("book_movie")) {
                    result = verobj.bookMovieTickets(parameters.get(0), parameters.get(1), parameters.get(2),
                            Integer.parseInt(parameters.get(3))); // issue
                } else if (func.equals("get_movieslots")) {
                    List<String> data = new ArrayList<String>();
                    String[] datastringarray = new String[256];
                    datastringarray = verobj.getBookingSchedule(parameters.get(0));
                    data = Arrays.asList(datastringarray);
                    result = String.join(",", data).trim();
                } else if (func.equals("cancel_movie")) {
                    if (parameters.size() == 3)
                        result = verobj.cancelMovieTickets(parameters.get(0), parameters.get(1),
                                Integer.parseInt(parameters.get(2)));
                    if (parameters.size() == 4)
                        result = verobj.cancelMovieTickets(parameters.get(0), parameters.get(1), parameters.get(2),
                                Integer.parseInt(parameters.get(3)));
                } else if (func.equals("increase_tickets")) {
                    verobj.increaseBookingCapacity(parameters.get(0), parameters.get(1));
                }
