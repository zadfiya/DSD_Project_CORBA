...
class ATWImpl extends DMTBSPOA {
    ...
    public String[] listMovieShowsAvailability(String movieName) {
        ...
        String outremont_res = requestAnotherServer("list_movieslots", params, out_port);
        List<String> ver_list = Stream.of(verdun_res.split(",", -1)).collect(Collectors.toList());
        List<String> out_list = Stream.of(outremont_res.split(",", -1)).collect(Collectors.toList());
        if (ver_list.size() > 1) {
            result.addAll(ver_list);
        }
        if (out_list.size() > 1) {
            result.addAll(out_list);
        }
        // Server Log
        try {
    ...
    public void serverLog(String user, String requestType, String params, String status, String result) throws IOException {
        ...
        log_file = log_dir + "\\src\\Replica3\\log\\server\\atwater_logs.txt"; // log file path
    ...
    public String bookMovieTickets(String customerID, String movieID, String movieName, int numberOfTickets) {
        ...
        if ((movieID.substring(0, 3)).equals("ATW")) {
            ...
            result = requestAnotherServer("book_movie", params, out_port);
            if (result.equals("Movie booked Successfully"))
                status = "Success";
            else
                status = "Failed";
    ...
    public String[] getBookingSchedule(String customerID) {
        ...
        for (Map.Entry mapElement : app_data.entrySet()) {
            ...
            for (Map.Entry mapSubElement : temp.entrySet()) {
                ...
                while (cap_data.contains(customerID)) {
                    if (!(result.contains("Atwater Movies : "))) {
                        result.add("Atwater Movies : ");
                    }
        ...
        if ((customerID.substring(0, 3)).equals("ATW")) {
            ...
            String out_res = requestAnotherServer("get_movieslots", params, out_port);
            List<String> ver_list = Stream.of(ver_res.split(",", -1)).collect(Collectors.toList());
            List<String> out_list = Stream.of(out_res.split(",", -1)).collect(Collectors.toList());
            if (ver_list.size() > 1) {
                result.addAll(ver_list);
            }
            if (out_list.size() > 1) {
    ...
    public String cancelMovieTickets(String customerID, String movieID, int numberOfTickets) {
        ...
        if ((movieID.substring(0, 3)).equals("ATW")) {
                ...
                for (Map.Entry mapSubElement : temp.entrySet()) { // check Movie ID is there or not
                    ...
                    if (appID.equals(movieID)) { // if Movie ID is there check the right customer is trying to
                        ...
                        if (cap_data.contains(customerID)) { // if yes then
                            ...
                            result = "the Movie booking has been cancelled for the customer " + customerID;
                            status = "Success";
                        } else {
                            result = "the Movie booking is not booked by " + customerID + " so he/she cannot cancel it ";
                            status = "Failed";
                        }
                        flag = 1;
                        break;
            ...
            result = requestAnotherServer("cancel_movie", params, out_port);
            if (result.equals("the Movie ticket has been cancelled for the customer " + customerID))
                status = "Success";
            else
                status = "Failed";
    ...
    public void increaseBookingCapacity(String movieID, String movieName) {
        ...
        if ((movieID.substring(0, 3)).equals("ATW")) {
            ...
            requestAnotherServer("increase_tickets", params, out_port);
        }
    }
...
public class AtwaterServer {
    ...
    public static void main(String[] args) {
        ...
        try {
            ...
            ATWImpl atwImpl = new ATWImpl();
            atwImpl.setORB(orb);

            // get object reference from the servant
            org.omg.CORBA.Object ref = rootpoa.servant_to_reference(atwImpl);
            ...
            Runnable UDPServer = new Runnable() {
                ...
                public void run() {
                    // TODO Auto-generated method stub
                    receiveRequestFromAnotherServer(atwImpl);
                }
            };
    ...
    public static void receiveRequestFromAnotherServer(ATWImpl atwobj) {
        String result = "";
        DatagramSocket ds = null;
        try {
            ds = new DatagramSocket(7981);
            byte[] receive = new byte[5000];
            System.out.println("UDP Server for Atwater is running on port : 7981");
            while (true) {
                ...
                if (func.equals("list_movieslots")) {
                    ...
                    ls = atwobj.listMovieAvailabilitySupport(parameters.get(0));
                    result = String.join(",", ls).trim();
                } else if (func.equals("book_movie")) {
                    result = atwobj.bookMovieTickets(parameters.get(0), parameters.get(1), parameters.get(2),
                            Integer.parseInt(parameters.get(3))); // issue
                } else if (func.equals("get_movieslots")) {
                    List<String> data = new ArrayList<String>();
                    String[] datastringarray = new String[256];
                    datastringarray = atwobj.getBookingSchedule(parameters.get(0));
                    data = Arrays.asList(datastringarray);
                    result = String.join(",", data).trim();
                } else if (func.equals("cancel_movie")) {
                    if (parameters.size() == 3)
                        result = atwobj.cancelMovieTickets(parameters.get(0), parameters.get(1),
                                Integer.parseInt(parameters.get(2)));
                    if (parameters.size() == 4)
                        result = atwobj.cancelMovieTickets(parameters.get(0), parameters.get(1), parameters.get(2),
                                Integer.parseInt(parameters.get(3)));
                } else if (func.equals("increase_tickets")) {
                    atwobj.increaseBookingCapacity(parameters.get(0), parameters.get(1));
                }
